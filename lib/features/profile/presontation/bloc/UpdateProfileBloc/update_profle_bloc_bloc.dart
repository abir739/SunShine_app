...
import 'package:SunShine/features/profile/domain/usecases/getuserusecase.dart';
import 'package:SunShine/features/profile/domain/usecases/updateuserusecase.dart';
  ...
  UpdateProfleBlocBloc({
    ...
    on<UpdateProfleBlocEvent>((event, emit) async {
      if (event is UpdateProfileEvent) {
        ...
        final FailurOrNotifications = await updateUserUseCase(event.user);

        FailurOrNotifications.fold((failure) {
          emit(ErrorUpdateProfileState(message: _mapfailureMassege(failure)));
        }, (_) {
